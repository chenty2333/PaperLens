...
def test_output_validation_rejects_non_consumable_core_v2_manifest(tmp_path):
    output_dir = tmp_path / "out"
    data_dir = output_dir / ".paperlens" / "data"
    paper = PaperRecord(
        paper_id="p_test",
    ...
    write_typed_artifact(
        data_dir / "core" / "v2" / "p_test" / "reading_plan.v1.json",
        artifact_type="reading_plan",
        data=full_plan.model_dump(),
        producer="unit_test_stale_plan",
        metadata={"paper_id": paper.paper_id},
    )
    write_core_v2_manifest(data_dir / "core" / "v2" / "p_test", "p_test")
    (output_dir / "papers").mkdir(parents=True)
    (output_dir / ".paperlens" / "library" / "index").mkdir(parents=True)
    (output_dir / ".paperlens" / "data" / "memory" / "v3").mkdir(parents=True)
    (output_dir / "PaperLens.md").write_text(
    ...
    with pytest.raises(RuntimeError, match="Core v2 manifest is not consumable"):
        validate_paperlens_output(
            output_dir,
            expected_report_names={"p_test.md"},
            expected_paper_ids={"p_test"},
